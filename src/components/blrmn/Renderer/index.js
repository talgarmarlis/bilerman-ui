...
const Index = ({ body }) => {
      ...
      <div className={`card ${withBorder ? '' : 'border-0'} ${withBackground ? 'bg-light' : ''}`}>
        <div className="card-body">
          <div className="d-flex mb-3">
            <img
              className={`img-fluid ml-auto mr-auto ${stretched ? 'width-100p' : ''}`}
              src={`${config.apiUrl}/images/article/${file.id}`}
              alt={caption}
            />
          </div>
          {getCaption(caption)}
  ...
  const getCode = ({ code }) => {
    return (
      <div className="card border-0 bg-light">
        <div className="card-body">
          <pre>
            <code>{getHTML(hljs.highlightAuto(code).value)}</code>
          </pre>
        </div>
      </div>
    )
  ...
  const getRaw = ({ html }) => {
    return (
      <div className="card border-0 bg-light">
        <div className="card-body">
          <pre>
            <code>{getHTML(hljs.highlightAuto(html).value)}</code>
          </pre>
        </div>
      </div>
    )
      ...
      <div className="card border-0">
        <div className="card-body">
          <ul className="list-unstyled">
            {items &&
              items.map(item => (
                <li className={styles.item} key={item + item}>
                  <label
                    htmlFor="checkbox-1"
                    className={`${styles.control} ${styles.checkbox} mb-0`}
                  >
  ...
  const getEmbed = ({ service, embed, width, height, caption }) => {
    ...
    return (
      <div key={`embed_${embed}`} className="card border-0">
        <div className="card-body">
          <div className="mb-3">{embedContent}</div>
          {getCaption(caption)}
        </div>
      </div>
    )
  ...
  return <>{body && body.blocks.map(block => renderSwitch(block))}</>
}
