...
const Details = ({
  article,
  article: { id, title, subtitle, imageId, tags, body, createdAt, user, views, comments },
}) => {
  return (
      ...
      <div className="card-body">
        ...
        {imageId && (
          <div className="d-flex">
            <img
              ...
              className="img-fluid mb-4 ml-auto mr-auto width-100p"
              src={`${config.apiUrl}/images/article/${imageId}`}
              alt={title}
            />
          </div>
        )}
          ...
          <div className="mb-3">
            {tags.map(tag => (
              <Link
                ...
                to={`/tags/${tag.name}`}
                className="badge text-blue text-capitalize bg-light font-size-16 mr-2 mb-2"
              >
                #{tag.name}
              </Link>
            ))}
        ...
        <div className="d-flex border-bottom-0 align-items-center">
          <div className="d-flex mr-auto">
            <span className="mr-4">
              <ViewCount count={views} type="default" />
            </span>
            <span className="mr-4">
              <CommentCount count={comments} type="default" />
            </span>
            <span>
              <Clap article={article} type="default" />
            </span>
          </div>
          <div className="mt-3">
            <Bookmark articleId={id} type="default" />
