...
const Comments = ({user, article, intl: { formatMessage }}) => {
  ...
  const updateComment = (comment) => {
    return commentService.saveComment(comment).then(result => {
      const {data} = result
      const index = comments.findIndex(function f(c) {return c.id === data.id;});
    ...
    <div className="card border-0 shadow-none">
      <div className="card-body">
        ...
        <Form className="login-form" onFinish={onFinish} form={form}>
          ...
          >
            <TextArea rows={3} placeholder={formatMessage({ id: 'article.details.comments.commentMessage'})} />
          </Form.Item>
          <Form.Item>
            <Button className="float-right btn btn-default" loading={loading} htmlType="submit" style={{ width: 150 }}>
        ...
        <h6 className="mb-4 text-uppercase">
          <strong><FormattedMessage id="article.details.comments.comments" /> ({article.comments})</strong>
        </h6>
        {comments.map(comment => (<Comment key={`comment_${comment.id}`} comment={comment} deleteComment={deleteComment} updateComment={updateComment} />))}

        {page && page.totalPages > 1 && page.number + 1 < page.totalPages &&
        <Button className="btn btn-default btn-block mt-3" onClick={() => loadComments(page.number + 1)} loading={loadingComments}>
